# -*- coding: utf-8 -*-
"""
stl_layered_concrete_fea_fastv10_prealloc.py  (V10)

V10 = V9 + persistent preallocated mechanics assembly buffer.

What changed from V9:
- The active-topology cache now allocates the mechanics COO value array once when the
  active element topology changes.
- Each mechanics solve refills that cached NumPy buffer in-place instead of growing a
  Python list with data.extend(...), and without allocating a fresh value array every time.
- Mechanical solve frequency is unchanged.

Key mechanics strategy retained from V9:
- Precompute one reference HEX8 stiffness/load template for the uniform structured mesh.
- Rebuild active topology only when the printed/connectivity-qualified active set changes.
- Refresh matrix values every mechanics solve because E still evolves with hydration.
- Impose zero Dirichlet BCs by solving only on free DOFs.

Run:
python stl_layered_concrete_fea_fastv10_prealloc.py --stl statueofliberty2.stl --nx 20 --ny 20 --nz 80 --auto_fit
"""
...
def run_sim(stl_path: str,
    ...
    E_elem = np.full(ne, mat.E0, dtype=np.float64)
