# -*- coding: utf-8 -*-
"""
stl_layered_concrete_fea_fastv9.py  (V9)

V9 = V8 + precomputed uniform HEX8 mechanics templates + cached active topology
     + free-DOF solve for zero Dirichlet BCs (no row/column BC editing)
     + previous-active reuse between steps

Key V9 mechanics changes:
- Precompute one reference HEX8 stiffness/load template for the structured mesh:
    * Ke_unit      : stiffness for E = 1
    * fe_iso_unit  : isotropic eigenstrain load template for eps_iso = 1
    * fe_body_unit : body-force load template
- Rebuild active-node topology only when the active printed/connected set changes:
    * active nodes
    * node map
    * element dof maps
    * COO rows/cols structure
    * fixed/free DOFs
- Assemble only values each step:
    Ke_e = E_elem[e] * Ke_unit
    fe_e = -E_elem[e] * eps_iso[e] * fe_iso_unit + fe_body_unit
- Solve only on free DOFs. This preserves symmetry and avoids expensive LIL edits.

Notes:
- Symmetric half-assembly is intentionally left for the next version.
- Old layers are NOT numerically frozen because E evolves with hydration.
  So topology is cached, but matrix values are rebuilt from current E_elem every solve.
- Thermal model remains the same as V8.

Run:
python stl_layered_concrete_fea_fastv9.py --stl statueofliberty2.stl --nx 20 --ny 20 --nz 80 --auto_fit
"""
        ...
        for k in range(self.nz):
            for j in range(self.ny):
                for i in range(self.nx):
                    ...
                    layers.append(k)
        return np.array(nodes, dtype=np.float64), np.array(conn, dtype=np.int64), np.array(layers, dtype=np.int64)
...
def build_hex8_reference_templates(Xe_ref: np.ndarray, mat: MaterialParams, body_force: np.ndarray):
    """
    Reference templates on one representative element.
    Because the structured mesh is uniform and affine, this is valid for all elements.

    Returns
    -------
    Ke_unit     : stiffness template for E = 1
    fe_iso_unit : element load template corresponding to isotropic eps_iso = 1
                  such that fe_eigen = -E_elem * eps_iso_elem * fe_iso_unit
    fe_body     : body force template (already full magnitude, not scaled by E)
    """
    if Xe_ref.shape != (8, 3):
    ...
    J = Xe_ref.T @ hex8_dN_dxi(0.0, 0.0, 0.0)
    ...
    N_all = np.empty((len(_GPS_2), 8), dtype=np.float64)
    for igp, (xi, eta, zeta) in enumerate(_GPS_2):
        N_all[igp, :] = 0.125 * (1 + hex8_s[:, 0] * xi) * (1 + hex8_s[:, 1] * eta) * (1 + hex8_s[:, 2] * zeta)
    ...
    return Ke_unit, fe_iso_unit, fe_body
...
class MechTopologyCache:
    ...
    def build(self, mesh: StructuredHexMesh, active: np.ndarray, bottom_nodes: np.ndarray):
        ...
        rows = np.empty(ne_valid * 24 * 24, dtype=np.int64)
        cols = np.empty(ne_valid * 24 * 24, dtype=np.int64)
...
# =========================================================
# Mechanics assembly using templates + cached topology
# =========================================================
def assemble_mech_active_submesh_v9(cache: MechTopologyCache,
                                    E_elem: np.ndarray,
    ...
    ne_valid = cache.active_elems.size
    data = np.empty(ne_valid * 24 * 24, dtype=np.float64)
    f = np.zeros(ndof, dtype=np.float64)

    p = 0
    for idx, e in enumerate(cache.active_elems):
        Ke = E_elem[e] * Ke_unit
        fe = -E_elem[e] * eps_iso_elem[e] * fe_iso_unit + fe_body_unit

        dofs = cache.edofs[idx]
        f[dofs] += fe
        data[p:p + 24 * 24] = Ke.reshape(-1)
        p += 24 * 24
...
def run_sim(stl_path: str,
    ...
    # Precompute one reference element template
    Xe_ref = mesh.nodes[mesh.conn[0]]
    Ke_unit, fe_iso_unit, fe_body_unit = build_hex8_reference_templates(Xe_ref, mat, body_force)
    print("[mechanics] Precomputed reference HEX8 templates.")
    ...
    for step, t in enumerate(times):
        ...
        if not active.any():
            ...
            K, f = assemble_mech_active_submesh_v9(
                cache=mech_cache,
                E_elem=E_elem,
        ...
        if step % 10 == 0 or step == len(times) - 1:
            ...
            print(f"t={t:8.1f}s | active_layers={active_layers:3d}/{nz} | max|u|={umax:.3e} m | "
                  f"Tavg={T.mean():.2f} K | alpha_avg(active)={ah:.3f} | topo={rebuilt_msg}")
