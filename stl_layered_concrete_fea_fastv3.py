# -*- coding: utf-8 -*-
"""
Created on Mon Feb 23 17:23:47 2026
...
# -*- coding: utf-8 -*-
"""
stl_layered_concrete_fea_fastv3.py

V3 = V2 + full-domain VTI with NaN masking (outside solid / outside active)
- Mechanics solved on ACTIVE-NODE SUBMESH (avoids singular K)
- Thermal + hydration use explicit SUBSTEPPING for stability
...
- VTI output keeps FULL structured domain, but:
    ...
    * Cell active is NaN outside SOLID cells (optional but useful)

Run:
python stl_layered_concrete_fea_fastv3.py --stl statueofliberty2.stl --nx 20 --ny 20 --nz 80 --auto_fit
"""
...
def hydration_rate(alpha_h, T, mat: MaterialParams):
    ...
    expo = np.clip(expo, -60.0, 60.0)  # exp(60) already enormous
    arr = np.exp(expo)
...
def assemble_mech_active_submesh(mesh: StructuredHexMesh,
    ...
    for e in range(mesh.nelems):
        ...
        for (xi, eta, zeta), wi in zip(gps, w):
            ...
            # body force (simple nodal distribution via shape functions)
...
# =========================================================
# Strong Dirichlet BC (adjust RHS for nonzero values)
# =========================================================
...
def _vtk_dataarray_ascii(name, ncomp, data_flat, dtype="Float32"):
    # Keep "nan" tokens (ParaView understands them in ASCII VTK XML)
    txt = " ".join("nan" if (isinstance(x, float) and np.isnan(x)) else f"{x:.7e}" for x in data_flat)
    return (
...
def run_sim(stl_path: str,
    ...
    # full-domain displacement buffer (we solve on submesh, then scatter here)
    ...
    for step, t in enumerate(times):
        ...
        # -------------------------
        active_nodes = None  # keep for VTI masking
        n_active_nodes = 0
        ...
        # -------------------------
        # VTI OUTPUT (FULL DOMAIN + NaN masking)
        # -------------------------
        if numerical_layer > last_output and active_layers > 0:
            ...
            # Cell data: active NaN outside SOLID cells (useful for Threshold)
            ...
            write_vti(
                ...
                cell_data={"alpha_h": alpha_vtk, "active": active_vtk, "solid": solid.astype(np.float32)}
            )
    ...
    print("Done. Load .vti in ParaView (Threshold on solid/active; Warp By Vector on U).")
