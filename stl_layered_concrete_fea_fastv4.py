# -*- coding: utf-8 -*-
"""
Created on Mon Feb 23 18:23:25 2026
...
# -*- coding: utf-8 -*-
"""
stl_layered_concrete_fea_fastv4.py  (V4)

V4 = V3 + ParaView .vti.series time-series file (works even with complex filenames)
- Keeps V3 output filenames like: step_0000_layers001.vti, step_0001_layers002.vti, ...
- Writes/updates a JSON file: sim.vti.series (ParaView file-series format)
  so ParaView 6.0.1 loads the whole sequence as a time-series.

Other V3 features kept:
- Mechanics solved on ACTIVE-NODE SUBMESH (avoids singular K)
- Thermal + hydration explicit SUBSTEPPING for stability
- Arrhenius overflow protection (clamp T + exponent)
- Thermal boundary = strong ambient Dirichlet (debug-stable)
- VTI output keeps FULL structured domain, but:
    * Point T is NaN outside SOLID geometry (node_in_solid=False)
    * Point U is NaN outside ACTIVE region at that time step
    * Cell alpha_h is NaN outside SOLID cells
    * Cell active is NaN outside SOLID cells

How to load:
- In ParaView, open: out/sim.vti.series

Run:
python stl_layered_concrete_fea_fastv4.py --stl statueofliberty2.stl --nx 20 --ny 20 --nz 80 --auto_fit
"""
...
def _vtk_dataarray_ascii(name, ncomp, data_flat, dtype="Float32"):
    # Ensure NaNs are written as "nan" tokens; ParaView understands this in ASCII XML VTK.
    out = []
    for x in data_flat:
        xx = float(x)
        if np.isnan(xx):
...
def write_vti_series(series_path: str, entries):
    """
    ParaView JSON series format:
    {
      "file-series-version": "1.0",
      "files": [{"name":"step_0000_layers001.vti","time":0.0}, ...]
    }
    entries: list[dict(name=str, time=float)]
    """
...
def run_sim(stl_path: str,
    ...
    # Precompute node mask: nodes that belong to ANY solid element (ever)
    node_in_solid = np.zeros(mesh.nnodes, dtype=bool)
    for e in np.where(solid)[0]:
        node_in_solid[mesh.conn[e]] = True
    ...
    # full-domain displacement buffer (solve submesh then scatter)
    ...
    for step, t in enumerate(times):
        ...
        # newly activated -> print temperature (outer step)
        ...
        for _ in range(nsub):
            ...
            # update modulus inside substeps
        ...
        # -------------------------
        active_nodes = None  # for VTI masking
        n_active_nodes = 0
        ...
        if not active.any():
            ...
            u_sub = spsolve(Kbc, fbc)  # length 3*n_active_nodes

            # scatter back to full domain
            u_full[:] = 0.0
        ...
        # -------------------------
        # VTI OUTPUT (FULL DOMAIN + NaN masking) + .vti.series update
        # -------------------------
        if numerical_layer > last_output and active_layers > 0:
            last_output = numerical_layer

            # Point data: T is NaN outside SOLID geometry
            T_vtk = T.copy()
            T_vtk[~node_in_solid] = np.nan

            # Point data: U is NaN outside ACTIVE region (current time)
            U = u_full.reshape((nn, 3)).copy()
            node_in_active = np.zeros(nn, dtype=bool)
            if active_nodes is not None and n_active_nodes > 0:
                node_in_active[active_nodes] = True
            U[~node_in_active, :] = np.nan
            Umag = np.linalg.norm(U, axis=1)

            # Cell data: alpha_h NaN outside SOLID cells
            alpha_vtk = alpha_h.copy()
            alpha_vtk[~solid] = np.nan

            # Cell data: active NaN outside SOLID cells
            active_vtk = active.astype(np.float32)
            active_vtk[~solid] = np.nan

            # Optional: store layer count as cell data
            active_layers_cell = np.full(mesh.nelems, float(active_layers), dtype=np.float32)
            active_layers_cell[~solid] = np.nan
            ...
            write_vti(
                ...
                dx=dx, dy=dy, dz=dz,
                origin=origin,
                point_data={"T": T_vtk, "U": U, "Umag": Umag},
                cell_data={
                    "alpha_h": alpha_vtk,
                    "active": active_vtk,
                    "solid": solid.astype(np.float32),
                    "active_layers": active_layers_cell,
                }
            )

            # Update series file (relative filenames recommended)
        ...
        if step % 10 == 0 or step == len(times) - 1:
            umax = float(np.nanmax(np.abs(u_full))) if u_full.size else 0.0
    ...
    print("Done.")
    print(f"Open this in ParaView: {series_path}")
