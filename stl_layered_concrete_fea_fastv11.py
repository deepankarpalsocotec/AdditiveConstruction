# -*- coding: utf-8 -*-
"""
stl_layered_concrete_fea_fastv11_stress.py  (V11)

V11 = V10 + fast cell-stress postprocessing written to VTI output.

What changed from V10:
- Stress is computed only when a VTI snapshot is written.
- Because the structured HEX8 mesh is uniform, the element-center B-operator is
  precomputed once on a reference element and reused for all cells.
- With fixed Poisson ratio, C(E, nu) = E * C_unit, so stress is recovered cheaply as:
      sigma = E_elem * C_unit @ (B_center @ u_e - eps_iso * I)
- VTI output now includes cell stresses:
      sigma_xx, sigma_yy, sigma_zz, sigma_xy, sigma_yz, sigma_xz, sigma_vm

Run:
python stl_layered_concrete_fea_fastv11.py --stl statueofliberty2.stl --nx 20 --ny 20 --nz 80 --auto_fit
"""
...
def compute_cell_stress_active(cache: MechTopologyCache,
    ...
    sigma_act = (strain_mech @ C_unit.T) * E_elem[cache.active_elems, None]
