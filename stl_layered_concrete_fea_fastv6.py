# -*- coding: utf-8 -*-
"""
stl_layered_concrete_fea_fastv6.py  (V6)

V6 = V5 + generalized total-strain (eps_tot) eigenstrain formulation for mechanical stresses;
...
Run:
python stl_layered_concrete_fea_fastv6.py --stl statueofliberty2.stl --nx 20 --ny 20 --nz 80 --auto_fit
"""
...
def assemble_mech_active_submesh(mesh: StructuredHexMesh,
    ...
    for e in range(mesh.nelems):
        ...
        for (xi, eta, zeta), wi in zip(gps, w):
            ...
            Ke += B.T @ C @ B * dV
            fe -= B.T @ (C @ eps_star) * dV  # eigenstrain enters with negative sign in B^T sigma
