"""
Unified fast code (Numba voxelization) with:
1) First-octant in X,Y only (shift STL so xmin,ymin -> pad; origin x=y=0)
2) Z is shifted so the *part bottom* (zmin) lands exactly at z=0 (no lift),
   and padding (if any) is applied on the +Z side (domain top) only.
   => The part always touches the build plate at z=0, avoiding singular K from floating base.

3) Correct strong Dirichlet enforcement:
   - adjusts RHS by subtracting K[:,d]*u_d
   - zeros row+column
   - sets diagonal to 1 and RHS to u_d

This avoids the prior singularity mechanism (floating due to pad/shift).
Disconnected islands can still create singularities physically, but with this Z policy,
early layers are far less likely to float.

Conda installs:
  conda create -n ac_fea -c conda-forge python=3.11 numpy scipy numba
  conda activate ac_fea
Optional:
  conda install -c conda-forge pyvista vtk

Run (recommended):
  python stl_layered_concrete_fea_fast_xy_octant_z0.py --stl part.stl --auto_fit --pad 0.002 --nx 120 --ny 120 --nz 250

Notes:
- Internal units SI (meters). STL assumed mm by default and scaled.
- Outputs VTI to out/*.vti
"""
...
# =========================================================
# Numba-accelerated voxelization (odd-even rule, +Z ray)
# =========================================================
@njit(inline="always")
        ...
        for k in range(self.nz):
            for j in range(self.ny):
                for i in range(self.nx):
                    ...
                    layers.append(k)
        return nodes, np.array(conn, dtype=int), np.array(layers, dtype=int)
...
def hydration_rate(alpha_h, T, mat: MaterialParams):
    ...
    arr = np.exp((-mat.E_a/mat.R) * (1.0/T - 1.0/mat.T_ref))
...
# Assembly
# =========================================================
def assemble_mech(mesh: StructuredHexMesh,
...
# =========================================================
# Strong Dirichlet with RHS adjustment (your "-K*d" request)
# =========================================================
def apply_dirichlet_strong(K: csr_matrix, f: np.ndarray, fixed_dofs: np.ndarray, values: np.ndarray):
    """
    Enforce u[d]=val with:
      f <- f - K[:,d]*val
      zero row/col d
      set K[d,d]=1 and f[d]=val
    """
    K = K.tolil()
...
# =========================================================
# XY first octant + Z bottom to 0, with padding
# =========================================================
...
def fit_domain_xy_octant_z0(tris_m: np.ndarray, pad_m: float):
    """
    Enforces:
      - x,y shifted so xmin->pad, ymin->pad, and origin x=y=0
      - z shifted so zmin->0 exactly (touch build plate), NOT pad.
      - domain extents:
          Lx = (xmax_shifted + pad)   (pad on +x side)
          Ly = (ymax_shifted + pad)
          Lz = (zmax_shifted + pad)   (pad on +z side only)
      - origin = (0,0,0)
    Returns: (tris_shifted, origin, Lx, Ly, Lz)
    """
    mn, mx = compute_bbox(tris_m)

    # shift x,y to first octant with pad gap to side walls
    sx = -(mn[0] - pad_m)
    sy = -(mn[1] - pad_m)

    # shift z so bottom touches plate exactly
    sz = -mn[2]
...
def run_sim(stl_path: str,
    ...
    if auto_fit:
        ...
        print(f"Auto-fit (XY octant, Z bottom=0). Origin={origin}, L=({Lx:.4f},{Ly:.4f},{Lz:.4f}) m, pad={pad_m} m")
        print(f"Applied shift (m): dx={shift[0]:.6f}, dy={shift[1]:.6f}, dz={shift[2]:.6f}")
    else:
        # Even without auto_fit, ensure zmin->0 and x/y octant shift to match origin=(0,0,0)
        # (Assumes user provides domain with origin (0,0,0) large enough.)
        tris_m, _, _, _, _, shift = fit_domain_xy_octant_z0(tris_m, pad_m=pad_m)
        origin = (0.0, 0.0, 0.0)
        print(f"Manual domain requested, but STL still shifted to XY-octant and Z-bottom=0. Shift (m): {shift}")
    ...
    # Build plate: z=0 exactly
    bottom_nodes = np.where(np.isclose(mesh.nodes[:, 2], 0.0))[0]
    fixed_dofs = np.array([3*n + d for n in bottom_nodes for d in (0, 1, 2)], dtype=int)
    fixed_vals = np.zeros_like(fixed_dofs, float)
    print(f"[BC] Build plate fixed at z=0 using {bottom_nodes.size} nodes.")
    ...
    for step, t in enumerate(times):
        active = (t >= elem_print_time) & solid

        # set print temp on newly active
        ...
        alpha_h = alpha_h + proc.dt * dadt * active
        alpha_h = np.clip(alpha_h, 0.0, mat.alpha_max)
        ...
        lapT = laplacian_structured_node(T, mesh.nx, mesh.ny, mesh.nz, mesh.dx, mesh.dy, mesh.dz)
        T = T + proc.dt * ((mat.kappa/(mat.rho*mat.cp)) * lapT + (q_node/(mat.rho*mat.cp)))

        # weak ambient relaxation at outer boundary (x=0/x=Lx, y=0/y=Ly, z=Lz)
        boundary = (
            np.isclose(mesh.nodes[:, 0], 0.0) | np.isclose(mesh.nodes[:, 0], mesh.Lx) |
            np.isclose(mesh.nodes[:, 1], 0.0) | np.isclose(mesh.nodes[:, 1], mesh.Ly) |
            np.isclose(mesh.nodes[:, 2], mesh.Lz)
        )
        T[boundary] += 0.02 * (proc.T_amb - T[boundary])

        # update Te for eigenstrain
        for e in range(ne):
            Te[e] = T[mesh.conn[e]].mean()
        ...
        # mechanical solve
        K, f = assemble_mech(mesh, active, E_elem, eigenstrain, mat, body_force=body_force)
        ...
        if step % 10 == 0 or step == len(times) - 1:
            umax = float(np.max(np.abs(u))) if np.all(np.isfinite(u)) else float(np.nanmax(np.abs(u)))
            ah = float(alpha_h[active].mean()) if active.any() else 0.0
            print(f"t={t:8.1f}s | active_layers={active_layers:3d}/{mesh.nz} | max|u|={umax:.3e} m | "
...
def main():
    ...
    ap.add_argument("--auto_fit", action="store_true", help="Auto-fit domain to STL with XY-octant and z-bottom=0")
    ap.add_argument("--pad", type=float, default=0.002, help="Padding in meters (applied in x,y as gap; in z on top side)")
    ap.add_argument("--layers_per_output", type=int, default=2)
    ap.add_argument("--out", type=str, default="out")

    # If not auto_fit, you can still pass domain sizes (meters). Origin forced to (0,0,0) by design.
    ap.add_argument("--Lx", type=float, default=0.2)
    ap.add_argument("--Ly", type=float, default=0.2)
